...
def my_reduce(my_input_stream, my_output_stream, my_reducer_input_parameters):
    ...
    for line in my_input_stream:
        ...
        if keyVal[0] not in dictionary:
            ...
            added = False
            x = minute-int(my_reducer_input_parameters[0])
            if x >= 0:
                if myList[-1][0] == hour and myList[-1][1] == x:
                    myList.append([hour,minute])
                    dictionary[keyVal[0]][-1].append([keyVal[1]])
                    added = True
            else:
                if myList[-1][0] == hour -1 and myList[-1][1] == 60 + x:
                    myList.append([hour,minute])
                    dictionary[keyVal[0]][-1].append([keyVal[1]])
                    added = True
            if not added:
                myList = [[hour, minute]]
                dictionary[keyVal[0]].append([keyVal[1]])

    for key in dictionary:
        for time in dictionary[key]:
            my_str = key + "\t" + "(" + time[0] +  ", " + str(len(time)) + ")" + "\n"
