...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for line in my_input_stream:
        ...
        myList = items[4].split(' ')
        key = myList[0]

        if int(items[0]) == 0 and int(items[5]) == 0 and items[1] == my_mapper_input_parameters[0]:
            dictionary[myList[1]] = key

    for key in dictionary:
        my_str = str(dictionary[key]) + "\t" + "(" + key + ")" + "\n"
...
if __name__ == '__main__':
    ...
    output_file_example = "../../my_result/A01 - Hint3/1. my_map_simulation/map_bikeMon_20170204.csv"
